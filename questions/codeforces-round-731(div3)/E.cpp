...
signed main () {
	...
	while (q--) {
		int n, k;
		cin >> n >> k;
		vector<vector<int> > v(k, vector<int>(5)), v2, v3;		// {a, t, t-a, t+a, i}
		for (int i = 0; i < k; i++) {
			cin >> v[i][0]; v[i][4] = i;
		}
		for (int i = 0; i < k; i++) {
		...
		for (int i = 1; i <= n; i++) {
			temp[0] = i;
			int ind = lower_bound(v3.begin(), v3.end(), temp) - v3.begin();
			if (ind == v3.size()) {
				ans = v3[ind-1][1] + i - v3[ind-1][0];
				ind--;
			}
			else {
				ans = v3[ind][1] + v3[ind][0]-i;
			}
			if (ind) ans = min(ans, v3[ind-1][1] + i - v3[ind-1][0]);
			cout << ans << ' ';
		}
		cout << '\n';
