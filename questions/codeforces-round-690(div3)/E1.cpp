...
int main () {
	...
	while (t--) {
		...
		for (int i = 1; i <= n; i++) {
			if (i-2 > 0) ans += freq[i] * C(freq[i-2], 2);
			if (i-1 > 0) ans += freq[i] * C(freq[i-1], 2);
			if (i+1 <= n) ans += freq[i] * C(freq[i+1], 2);
			if (i+2 <= n) ans += freq[i] * C(freq[i+2], 2);
		}
		for (int i = 1; i <= n-2; i++) {
