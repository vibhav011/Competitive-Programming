...
signed main () {
	...
	while (t--) {
		...
		int c = min(r-n, 1-l)%MOD;
		int ans = (c*ncr(n, n/2))%MOD;
		...
		for (int i = c+1; i <= c+n+1; i++) {
			...
			int b = max(0LL, n+i-r);
			int mx = max(a, b);
			if (mx > (n+1)/2 || n-a-b < 0) break;
			if (n&1) {
				// cout << mx << ' ' << a << ' ' << b << '\n';
				if (mx <= n/2) ans = (ans + ncr(n-a-b, n/2-mx) + ncr(n-a-b, n/2+1-mx)) % MOD;
				else ans = (ans + 1) % MOD;
			}
			else {
				ans = (ans + ncr(n-a-b, n/2-mx)) % MOD;
			}
		}
		cout << ans << '\n';
	}
