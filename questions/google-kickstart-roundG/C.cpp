...
signed main () {
	...
	for (int cn = 1; cn <= t; cn++) {
		...
		vector<int> difs(k+1, 1LL << 50LL);
		int ans = 6000;

		vector<pii> to_add[n+1];

		for (int i = 0; i < n; i++) {
			// int j = lower_bound(pref.begin(), pref.end(), pref[i]+k) - pref.begin();
			// j = min(n, j);
			// if (pref[j] - pref[i] == k) ans = min(ans, j-i);
			
			for (int j = i; j < n; j++) {
				int alpha = pref[j+1] - pref[i];
				if (alpha > k) break;
				if (alpha == k) ans = min(ans, j+1-i);
				// difs[alpha] = min(difs[alpha], j+1-i);
				int rest = k-alpha;
				// if (i == 4 && j == 5) {
				// 	cout << difs[3] << '\n';
				// 	cout << alpha << ' ' <<  rest << '\n';
				// }
				ans = min(ans, (j+1-i) + difs[rest]);
				to_add[j].pb(mp(alpha, j+1-i));
			}
			for (pii p : to_add[i]) difs[p.ff] = min(difs[p.ff], p.ss);
		}
	// cout << difs[3] << '\n';
		cout << (ans==6000 ? -1 : ans) << '\n';
	}
