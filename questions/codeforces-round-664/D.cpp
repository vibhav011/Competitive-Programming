...
int main () {
	...
	sort(less.begin(), less.end(), greater<ll>());
	sort(more.begin(), more.end(), greater<ll>());
	for (int i = 1; i < less.size(); i++) less[i] += less[i-1];
	for (int i = 1; i < more.size(); i++) more[i] += more[i-1];
	...
	ll ans = (ceil(n/(d+1.0)) <= more.size()) ? more[ceil(n/(d+1.0))-1] : 0;
	// cout << ans << endl;
	for (int i = 1; i <= less.size(); i++) {
		ll a = less[i-1];
		if (ceil((n-i)/(d+1.0)) <= more.size()) {
			a += more[ceil((n-i)/(d+1.0))-1];
			ans = max(ans, a);
		}
	}
	cout << ans << endl;
