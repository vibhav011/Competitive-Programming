...
signed main () {
	...
	int pmod[60];
	for (int i = 0; i < 60; i++) pmod[i] = (1LL << i) % MOD;
	...
	while (t--) {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 0; j < 60; j++) {
				if (a[i]&cur) {
					aa = addmod(aa, mulmod(b[j], pmod[j]));
					rr = addmod(rr, mulmod(n, pmod[j]));
				}
				else rr = addmod(rr, mulmod(b[j], pmod[j]));
				cur <<= 1LL;
			}
			ans = addmod(ans, mulmod(aa, rr));
		}
		cout << ans << endl;
	}
}
