...
signed main () {
	...
	while (t--) {
		...
		for (int i = 0; i < n; i++) {
			...
			for (it = freq.begin(); it != it2; it++) {
				times[it->ff].pb(it->ss);
			}
			freq.erase(freq.begin(), it2);
		}
		for (it = freq.begin(); it != freq.end(); it++) {
			times[it->ff].pb(it->ss);
		}
		map<int, deque<int> > times2 = times;

		for (int i = 0; i < n; i++) {
			int x = times[h[i]].front();
			cout << times2[h[i]].front()-1 << ' ';
			times[h[i]].pop_front();
			x--;
			if (x) times[h[i]].push_front(x);
			else times2[h[i]].pop_front();
		}
		cout << '\n';
	}
}
